...
(* writes graph element info in a file for visualization*)
let export path graph =
  ...
  (* Edges LR_0 -> LR_2 [ label = "SS(B)" ]; *)
  v_iter graph (fun vi -> List.iter (fun (label, id2) -> out "LR_%s -> LR_%s [ label = \"SS(%s)\" ]; \n" vi.id id2 label) vi.outedges) ;
...
(* writes graph element info in a file*)
